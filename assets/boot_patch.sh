...
case $((STATUS & 3)) in
  0 )  # Stock boot
    ...
    ;;
  1 )  # Magisk patched
    # use real magisk binaries
    rm -rf magisk32 magisk64 magiskinit
    ui_print "- Patched boot image, keep Magisk if installed"
    ./magiskboot cpio ramdisk.cpio "extract overlay.d/sbin/magisk64.xz magisk64.xz"
    ./magiskboot cpio ramdisk.cpio "extract overlay.d/sbin/magisk32.xz magisk32.xz"
    ./magiskboot cpio ramdisk.cpio "extract init magiskinit"
    ./magiskboot cpio ramdisk.cpio "extract overlay.d/stub.xz stub.xz"
...
[ -f magisk64.xz ] && unset SKIP64
[ -f magisk32.xz ] && unset SKIP32
[ -f stub.xz ] && unset SKIPSTUB
...
find overlay.d | while read line; do
  if [ -d "$line" ]; then
      ./magiskboot cpio ramdisk.cpio "mkdir 0755 $line"
  else
      ./magiskboot cpio ramdisk.cpio "add 0755 $line $line"
  fi
done
